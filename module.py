...
class DAN():
    ...
    def __init__(self):
        ...
        variables_to_restore = [v for v in all_variables if
                                v.name.split('/')[0] == 'st_network']
    ...
    def create_attn_st_network(self):
        print("create_attn_st_network")
        with tf.variable_scope("st_network"):
            #     self.attn_mask = tf.placeholder(tf.float32, shape=(input_steps, input_steps))
            self.st_known_ = tf.placeholder(tf.int64,
                                            shape=(batch_size, input_steps),
                                            name='st_known')
            self.st_output_ = tf.placeholder(tf.int64,
                                             shape=(batch_size, input_steps),
                                             name='st_output')
            self.st_time = tf.placeholder(tf.int64,
                                          shape=(batch_size, input_steps),
                                          name='st_time')
            self.st_time_emb = tf.contrib.layers.embed_sequence(self.st_time,
                                                                time_num,
                                                                time_size,
                                                                scope="time_embedding")
            self.st_user = tf.placeholder(tf.int64, shape=(batch_size, 1),
                                          name='st_user')
            self.st_user_tile = tf.tile(self.st_user,
                                        [1, tf.shape(self.st_known_)[1]])
            self.st_user_emb = tf.contrib.layers.embed_sequence(
                self.st_user_tile, user_num, user_size, scope="user_embedding")

            self.st_known_embedding = tf.contrib.layers.embed_sequence(
                self.st_known_, block_num, loc_size,
                scope="location_embedding")
            self.st_destination_ = tf.placeholder(tf.int64,
                                                  shape=(batch_size, 1),
                                                  name='p_destination')
            self.st_destination_embedding = tf.contrib.layers.embed_sequence(
                self.st_destination_, block_num, loc_size,
                scope="location_embedding", reuse=True)
            cell, initial_state = self.build_lstm(tf.shape(self.st_known_)[0])
            fusion = tf.concat(
                [self.st_known_embedding, self.st_time_emb, self.st_user_emb],
                2)
            self.outputs, self.final_state = tf.nn.dynamic_rnn(cell, fusion,
                                                               initial_state=initial_state,
                                                               dtype=tf.float32)  # batch * len * dim
            ...
            #
            ################ history module
            self.his_user = tf.placeholder(tf.int64, shape=(
            batch_size, his_num, his_length), name='his_user')
            self.his_tra = tf.placeholder(tf.int64, shape=(
            batch_size, his_num, his_length), name='his_tra')
            self.his_time = tf.placeholder(tf.int64, shape=(
            batch_size, his_num, his_length), name='his_time')
            self.his_padding_mask = tf.placeholder(tf.float32, shape=(
            batch_size, his_num, his_length), name='his_mask')

            self.his_user_emb = tf.contrib.layers.embed_sequence(self.his_user,
                                                                 user_num,
                                                                 user_size,
                                                                 scope="user_embedding",
                                                                 reuse=True)
            self.his_tra_emb = tf.contrib.layers.embed_sequence(self.his_tra,
                                                                block_num,
                                                                lstm_size,
                                                                scope="location_embedding",
                                                                reuse=True)
            self.his_time_emb = tf.contrib.layers.embed_sequence(self.his_time,
                                                                 time_num,
                                                                 time_size,
                                                                 scope="time_embedding",
                                                                 reuse=True)
            self.his_tra_emb = tf.concat(
                [self.his_tra_emb, self.his_time_emb, self.his_user_emb],
                3)  # batch_size his_num his_length lstm_size
            self.his_tra_emb = tf.reshape(self.his_tra_emb, [-1, his_length,
                                                             lstm_size + time_size + user_size])

            self.initial_state_1 = cell.zero_state(
                tf.shape(self.his_tra_emb)[0], tf.float32)
            self.his_outputs, self.his_final_state = tf.nn.dynamic_rnn(cell,
                                                                       self.his_tra_emb,
                                                                       initial_state=self.initial_state_1,
                                                                       dtype=tf.float32)
            self.his_outputs = tf.reshape(self.his_outputs,
                                          [tf.shape(self.his_user)[0],
                                           tf.shape(self.his_user)[1],
                                           tf.shape(self.his_user)[2],
                                           lstm_size])

            # batch_size len lstm_size * batch_size lstm_size his_num his_len
            self.his_attn_1 = tf.einsum("iml,iljk->imjk", self.outputs,
                                        tf.transpose(self.his_outputs,
                                                     [0, 3, 1,
                                                      2]))  # batch len his_num his_len
            self.his_attn_1 = self.his_attn_1 * tf.tile(
                tf.expand_dims(self.his_padding_mask, 1),
                [1, tf.shape(self.st_known_)[1], 1, 1])
            # batch_size len his_num his_length * batch_size len his_num his_length lstm_size
            self.his_outputs = tf.einsum("ijkl,ijklm->ijkm", self.his_attn_1,
                                         tf.tile(
                                             tf.expand_dims(self.his_outputs,
                                                            1),
                                             [1, tf.shape(self.st_known_)[1],
                                              1, 1,
                                              1]))  # batch_size len his_num lstm_size
            # batch_size len lstm_size * batch_size len lstm_size his_num
            self.his_attn_2 = tf.einsum("ijk,ijkl->ijl", self.outputs,
                                        tf.transpose(self.his_outputs,
                                                     [0, 1, 3,
                                                      2]))  # batch_size len his_num
            self.his_outputs = tf.einsum("ijk,ijkl->ijl", self.his_attn_2,
                                         self.his_outputs)  # batch_size len lstm_size
            ...
            with tf.variable_scope('st_output'):
                w_p1 = tf.Variable(
                    tf.truncated_normal([3 * lstm_size, block_num],
                                        stddev=0.1))
                b_p1 = tf.Variable(tf.zeros(block_num))

            print(self.st_destination_embedding.shape, "--------------------")
            self.st_layer_1 = tf.nn.relu(tf.matmul(tf.reshape(tf.concat([
                                                                            tf.tile(
                                                                                self.st_destination_embedding,
                                                                                [
                                                                                    1,
                                                                                    tf.shape(
                                                                                        self.st_known_)[
                                                              ...
                                                              [-1,
                                                               3 * lstm_size]),
                                                   w_p1) + b_p1)
            self.st_all = tf.reshape(self.st_layer_1,
                                     [tf.shape(self.st_known_)[0],
                                      tf.shape(self.st_known_)[1], block_num])
            ...
            self.st_all_cost = tf.nn.softmax_cross_entropy_with_logits(
                logits=self.st_all, labels=action_one_hot)
            self.st_all_cost = tf.reduce_mean(self.st_all_cost)
            self.st_all_optimizer = tf.train.AdamOptimizer(0.0001).minimize(
                self.st_all_cost)

    def create_step_by_step_st_network(self):
        print("create_step_by_step_st_network")
        with tf.variable_scope("st_network"):
            self.st_known_ = tf.placeholder(tf.int64,
                                            shape=(batch_size, input_steps),
                                            name='st_known')
            self.neg_known_ = tf.placeholder(tf.int64,
                                             shape=(batch_size, input_steps),
                                             name='st_known')
            st_known_embedding = tf.contrib.layers.embed_sequence(
                self.st_known_, block_num, lstm_size,
                scope="location_embedding")
            neg_known_embedding = tf.contrib.layers.embed_sequence(
                self.neg_known_, block_num, lstm_size,
                scope="location_embedding", reuse=True)
            print("--------------", st_known_embedding)
            self.st_destination_ = tf.placeholder(tf.int64, shape=(batch_size),
                                                  name='p_destination')
            self.st_destination_embedding = tf.contrib.layers.embed_sequence(
                self.st_destination_, block_num, lstm_size,
                scope="location_embedding", reuse=True)
            cell, initial_state = self.build_lstm(tf.shape(self.st_known_)[0])
            self.states, self.final_state = tf.nn.dynamic_rnn(cell,
                                                              tf.transpose(
                                                                  st_known_embedding,
                                                                  [1, 0, 2]),
                                                              initial_state=initial_state,
            ...
            def compute(i, cur_state, out):
                output, cur_state = cell(neg_known_embedding[:, i + 1],
                                         self.states[i])
            ...
            with tf.variable_scope('st_output'):
                w_p1 = tf.Variable(
                    tf.truncated_normal([lstm_size, 2 * lstm_size],
                                        stddev=0.1))
                b_p1 = tf.Variable(tf.zeros(2 * lstm_size))

                w_p2 = tf.Variable(
                    tf.truncated_normal([2 * lstm_size, 1], stddev=0.1))
                b_p2 = tf.Variable(tf.zeros(1))

            self.cutted_states = self.states[1:-1]  # 截头去尾
            self.nest_outputs = tf.reshape(
                tf.add(self.st_destination_embedding, self.cutted_states),
                [-1, lstm_size])

            self.st_layer_1 = tf.nn.relu(
                tf.matmul(self.nest_outputs, w_p1) + b_p1)
            self.st = tf.matmul(self.st_layer_1, w_p2) + b_p2

            self.infer_outputs = tf.add(self.st_destination_embedding,
                                        self.states[-1, :, :])
            self.infer_st_layer_1 = tf.nn.relu(
                tf.matmul(self.infer_outputs, w_p1) + b_p1)
            self.infer_st = tf.nn.sigmoid(
                tf.matmul(self.infer_st_layer_1, w_p2) + b_p2)

            self.nest_neg_outputs = tf.reshape(
                tf.add(self.st_destination_embedding, out.stack()),
                [-1, lstm_size])
            self.neg_layer_1 = tf.nn.relu(
                tf.matmul(self.nest_neg_outputs, w_p1) + b_p1)
            self.neg = tf.matmul(self.neg_layer_1, w_p2) + b_p2

            self.st_cost = tf.reduce_mean(
                - tf.log(tf.nn.sigmoid(self.st)) - tf.log(
                    tf.nn.sigmoid(- self.neg)))
            ...
            self.st_optimizer = tf.train.AdamOptimizer(0.0001).minimize(
                self.st_cost)
    ...
    def history_attention(self, query, des):
        ...
        with tf.variable_scope("st_network"):
            ...
            def compute(i, cur_state, out, fin):
                his_state, final_state = tf.nn.dynamic_rnn(cell, tf.transpose(
                    self.his_tra_emb[:, i, :, :], [1, 0, 2]),
                                                           initial_state=initial_state,
                                                           dtype=tf.float32,
                                                           time_major=True)
                return i + 1, cur_state, out.write(i, his_state), fin.write(i,
                                                                            final_state)

            _, cur_state, out, fin = tf.while_loop(
                lambda a, b, c, d: a < his_num,
                compute,
                (0, initial_state, tf.TensorArray(tf.float32, his_num),
                 tf.TensorArray(tf.float32, his_num))
            )
            out = out.stack()
            # out  tra_num * tra_len * batch_size * lstm_size  -> batch_size * tra_num * tra_len * lstm_size  query batch_size * lstm_size
            weight_1 = tf.einsum('ijkl,il->ijk', tf.reshape(out, [
                tf.shape(self.his_tra_emb)[0], his_num, his_length,
                lstm_size]), query)
            ...
            weight_2 = tf.einsum('ijkl,il->ijk', self.his_tra_emb,
                                 tf.squeeze(des, [1]))
            ...
            local_weight = tf.nn.softmax(
                (weight_1 + weight_2) * tf.cast(self.his_padding_mask,
                                                dtype=tf.float32))  # batch_size his_num his_length
            tra_emb = tf.einsum('ijk,ijkl->ijl', local_weight,
                                tf.transpose(out, [2, 0, 1,
                                                   3]))  # batch_size * his_num  * his_length * lstm_size -> batch_size * his_num *lstm_size
            global_weight = tf.nn.softmax(
                tf.einsum('ijk,ik->ij', tra_emb, query))  # batch_size his_hum
        return tf.einsum('ij,ijk->ik', global_weight,
                         tra_emb)  # batch_size lstm_size

    def create_st_network(self):
        print("create_st_network")
        with tf.variable_scope("st_network"):
            self.st_known_ = tf.placeholder(tf.int64,
                                            shape=(batch_size, input_steps),
                                            name='st_known')
            ...
            self.st_output_ = tf.placeholder(tf.int64,
                                             shape=(batch_size, input_steps),
                                             name='st_output')
            self.trans_mat = tf.placeholder(tf.int64, shape=(
            batch_size, input_steps, 70), name='tras_mat')
            self.st_time = tf.placeholder(tf.int64,
                                          shape=(batch_size, input_steps),
                                          name='st_time')
            ...
            self.st_known_embedding = tf.contrib.layers.embed_sequence(
                self.st_known_, block_num, lstm_size,
                scope="location_embedding")
            self.neg_known_embedding = tf.contrib.layers.embed_sequence(
                self.neg_known_, block_num, lstm_size,
                scope="location_embedding", reuse=True)
            ...
            self.his_time = tf.placeholder(tf.int64, shape=(
            batch_size, his_num, his_length), name='his_time')
            self.his_day = tf.placeholder(tf.int64, shape=(
            batch_size, his_num, his_length), name='his_day')
            self.his_padding_mask = tf.placeholder(tf.int64, shape=(
            batch_size, his_num, his_length), name='his_mask')
            self.his_tra_emb = tf.contrib.layers.embed_sequence(self.his_tra,
                                                                block_num,
                                                                lstm_size,
            ...
            self.his_time_emb = tf.contrib.layers.embed_sequence(self.his_time,
                                                                 ...
                                                                 scope="time_embedding",
                                                                 reuse=True)
            self.his_day_emb = tf.contrib.layers.embed_sequence(self.his_day,
                                                                day_num,
                                                                day_size,
                                                                scope="day_embedding",
                                                                reuse=True)
            self.his_tra_emb = tf.concat(
                [self.his_tra_emb, self.his_time_emb, self.his_day_emb],
                3)  # batch_size his_num his_length lstm_size
            self.his_tra_emb = tf.contrib.layers.fully_connected(
                self.his_tra_emb, lstm_size)  # default relu activation
            ...
            def compute(i, cur_state, ctx):
                his_state = self.history_attention(self.outputs[i],
                                                   self.st_destination_embedding)
            ...
            with tf.variable_scope('st_output'):
                ...
                w_p2 = tf.Variable(
                    tf.truncated_normal([2 * lstm_size, block_num],
                                        stddev=0.1))
                b_p2 = tf.Variable(tf.zeros(1))

            print(self.st_destination_embedding, self.final_state[0][1],
                  self.outputs)
            self.st_layer_1 = tf.nn.relu(tf.matmul(tf.reshape(
                tf.add(tf.squeeze(self.st_destination_embedding, [1]),
                       self.outputs[:, :, :]), [-1, lstm_size]), w_p1) + b_p1)
            self.st_all = tf.transpose(
                tf.reshape(tf.matmul(self.st_layer_1, w_p2) + b_p2,
                           [tf.shape(self.st_known_)[1],
                            tf.shape(self.st_known_)[0], block_num]),
                [1, 0, 2])
            self.st_all_prob = tf.nn.softmax(self.st_all)
            self.mask_code = tf.cast(tf.reduce_sum(
                tf.one_hot(self.trans_mat, block_num, dtype=tf.uint8), axis=2),
                                     dtype=tf.bool)
            ...
            action_one_hot = tf.one_hot(self.st_output_, block_num)
            self.st_cost = tf.nn.softmax_cross_entropy_with_logits(
                logits=self.st_all, labels=action_one_hot)
    ...
    def attn_head(self, inp, bias_mat, context, geo_dist):
        ...
        with tf.variable_scope('attention_weight'):
            w_a1 = tf.Variable(
                tf.truncated_normal([lstm_size, 64], stddev=0.1))
    ...
    def geo_pgnn_layer(self, feature, anchor_sets, sub_sets, context, pf_w,
                       pf_b, out_w, out_b):
        print("geo_pgnn_layer")
        anchor_feature = tf.contrib.layers.embed_sequence(anchor_sets,
                                                          block_num, loc_size,
                                                          scope="st_network/location_embedding",
                                                          reuse=True)
        self_feature = tf.contrib.layers.embed_sequence(sub_sets, block_num,
                                                        loc_size,
                                                        scope="st_network/location_embedding",
                                                        reuse=True)
        ...
        context = tf.tile(
            tf.expand_dims(tf.expand_dims(context, axis=1), axis=1),
            [1, 2, anchor_num, 1])
        dist_compute = tf.gather(self.anchor_dist_mat, sub_sets)

        messages = anchor_feature * (
                    1 / (1 + tf.expand_dims(dist_compute, axis=3)))
    ...
    def pf_pgnn_layer(self, feature, anchor_sets, sub_sets, context, pf_w,
                      pf_b, out_w, out_b):
        print("pf_pgnn_layer")
        #    print(feature.shape)
        ...
        context = tf.tile(
            tf.expand_dims(tf.expand_dims(context, axis=1), axis=1),
            [1, 2, anchor_num, 1])
        dist_compute = tf.einsum('ijkl,l->ijk', tf.concat(
            [self_feature, anchor_feature, context], 3),
                                 pf_w) + pf_b  # batch_size 2 anchor_num
    ...
    def create_pgnn_heuristics_network(self):
        print("create_pgnn_heuristics_network")
        with tf.variable_scope("pf_network"):
            pf_w = tf.Variable(
                tf.truncated_normal([3 * lstm_size], stddev=0.1))
            pf_b = tf.Variable(tf.zeros(1))
        with tf.variable_scope("output_network"):
            out_w = tf.Variable(
                tf.truncated_normal([2 * lstm_size], stddev=0.1))
            out_b = tf.Variable(tf.zeros(1))
            out_pf_w = tf.Variable(
                tf.truncated_normal([2 * lstm_size], stddev=0.1))
        ...
        with tf.variable_scope("value_network"):
            with tf.variable_scope('value_output'):
                w_v1 = tf.Variable(
                    tf.truncated_normal([loc_size + 2 * lstm_size, lstm_size],
                                        stddev=0.1))
                b_v1 = tf.Variable(tf.zeros(lstm_size))
                w_v2 = tf.Variable(
                    tf.truncated_normal([4 * anchor_num + lstm_size + 1, 1],
                                        stddev=0.1))
                b_v2 = tf.Variable(tf.zeros(1))

        self.loc_embedding = tf.placeholder(tf.float32,
                                            shape=(None, lstm_size),
                                            name='all_emb')  # get tensor by name

        self.anchor_set = tf.placeholder(tf.int32,
                                         shape=(batch_size, anchor_num),
                                         name='src')
        self.sub_sets = tf.placeholder(tf.int32, shape=(batch_size, 2),
                                       name='sub_graph')
        self.start_geo = tf.placeholder(tf.float32, shape=(batch_size, 2),
                                        name="start_geo")
        self.end_geo = tf.placeholder(tf.float32, shape=(batch_size, 2),
                                      name="end_geo")
        self.sub_geo = self.end_geo - self.start_geo
        self.heu_dist = tf.sqrt(
            self.sub_geo[:, 0] * self.sub_geo[:, 0] + self.sub_geo[:,
                                                      1] * self.sub_geo[:, 1])
        ...
        self.value_layer_1 = tf.nn.relu(tf.matmul(tf.concat(
            [self.stop_outputs, tf.squeeze(self.stop_des_emb, axis=1)], 1),
                                                  w_v1) + b_v1)
        #    tf.concat([tf.tile(self.st_destination_embedding, [1, tf.shape(self.st_known_)[1], 1]),  self.outputs], 2)

        x = self.loc_embedding
        x_position, _ = self.geo_pgnn_layer(x, self.anchor_set, self.sub_sets,
                                            self.value_layer_1, pf_w, pf_b,
                                            out_w, out_b)
        p_position, _ = self.pf_pgnn_layer(x, self.anchor_set, self.sub_sets,
                                           self.value_layer_1, pf_w, pf_b,
                                           out_pf_w, out_pf_b)
        ...
        self.heuristics = tf.matmul(tf.concat(
            [x_position[:, -2, :], x_position[:, -1, :], p_position[:, -2, :],
             p_position[:, -1, :], self.value_layer_1, self.heu_dist], 1),
                                    w_v2) + b_v2
        #    self.heuristics = tf.matmul(self.value_layer_1, w_v2) + b_v2
        #####  supervised loss
        self.heuristics_input = tf.placeholder(tf.float32, shape=(batch_size),
                                               name='heuristics_input')
        self.heuristics_cost = tf.reduce_mean(
            tf.square(self.heuristics_input - self.heuristics))
        #####

        #      self.gradients = tf.gradients(self.heuristics_cost, [output_state])

        self.value_variables = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, "value_network")

        self.optimizer = tf.train.AdamOptimizer(0.0001).minimize(
            self.heuristics_cost, var_list=self.value_variables)
    ...
    def create_heuristics_network(self):
        print("create_heuristics_network")
        with tf.variable_scope("value_network"):
            with tf.variable_scope('value_output'):
                w_v1 = tf.Variable(
                    tf.truncated_normal([2 * lstm_size, lstm_size],
                                        stddev=0.1))
                b_v1 = tf.Variable(tf.zeros(lstm_size))
                w_v2 = tf.Variable(
                    tf.truncated_normal([lstm_size, 1], stddev=0.1))
                b_v2 = tf.Variable(tf.zeros(1))

            # self.outputs[-1, :, :]
            self.stop_des_emb = tf.stop_gradient(self.st_destination_embedding)
            self.stop_known_emb = tf.stop_gradient(
                self.st_known_embedding[:, -1, :])
            self.stop_outputs = tf.stop_gradient(self.rnn_outputs[:, -1, :])
            self.value_layer_1 = tf.nn.relu(tf.matmul(tf.concat(
                [self.stop_outputs, tf.squeeze(self.stop_des_emb, axis=1)], 1),
                                                      w_v1) + b_v1)

            self.src_bias_mat = tf.placeholder(tf.float32,
                                               shape=(batch_size, None, None),
                                               name='src_adj_mat')
            self.src_embedding = tf.placeholder(tf.float32, shape=(
            batch_size, None, lstm_size), name='src_emb')  # get tensor by name

            self.des_bias_mat = tf.placeholder(tf.float32,
                                               shape=(batch_size, None, None),
                                               name='des_adj_mat')
            self.des_embedding = tf.placeholder(tf.float32, shape=(
            batch_size, None, lstm_size), name='des_emb')  # get tensor by name

            self.src_geo_mat = tf.placeholder(tf.float32,
                                              shape=(batch_size, None, None),
                                              name='geo_src')
            self.des_geo_mat = tf.placeholder(tf.float32,
                                              shape=(batch_size, None, None),
                                              name='geo_des')
            ...
            for it in range(layers):  # layers 层数
                ...
                for _ in range(heads):  # head数
                    attns.append(self.attn_head(h_1, self.src_bias_mat,
                                                self.stop_outputs,
                                                self.src_geo_mat))
            ...
            for it in range(layers):  # layers 层数
                ...
                if it == 3:
                    self.des_f = h_2
                for _ in range(heads):  # head数
                    attns.append(self.attn_head(h_2, self.des_bias_mat,
                                                self.stop_outputs,
                                                self.des_geo_mat))
            ...
            self.heuristics = tf.matmul(self.value_layer_1 + h_1 + h_2,
                                        w_v2) + b_v2  # self.value_layer_1 + h_1 + h_2
            ...
            #####  supervised loss
            self.heuristics_input = tf.placeholder(tf.float32,
                                                   shape=(batch_size),
                                                   name='heuristics_input')
            self.heuristics_cost = tf.reduce_mean(
                tf.square(self.heuristics_input - self.heuristics))
    ...
    def old_create_heuristics_network(self):
        print("old_create_heuristics_network")
        with tf.variable_scope("value_network"):
            self.known_ = tf.placeholder(tf.int64,
                                         shape=(batch_size, input_steps),
                                         name='known')
            known_embedding = tf.contrib.layers.embed_sequence(self.known_,
                                                               block_num,
                                                               lstm_size,
                                                               scope="value_location_embedding")
            self.waiting_ = tf.placeholder(tf.int64, shape=(batch_size),
                                           name='waiting')
            waiting_embedding = tf.contrib.layers.embed_sequence(self.waiting_,
                                                                 block_num,
                                                                 lstm_size,
                                                                 scope="value_location_embedding",
                                                                 reuse=True)
            self.destination_ = tf.placeholder(tf.int64, shape=(batch_size),
                                               name='destination')
            destination_embedding = tf.contrib.layers.embed_sequence(
                self.destination_, block_num, lstm_size,
                scope="value_location_embedding", reuse=True)
            # network weights

            fw_cell, fw_initial_state = self.build_lstm(
                tf.shape(self.known_)[0])
            bw_cell, bw_initial_state = self.build_lstm(
                tf.shape(self.known_)[0])
            #    print("-------", tf.concat([known_embedding, tf.expand_dims(waiting_embedding, 1)], 1))
            outputs, state = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell,
                                                             tf.transpose(
                                                                 tf.concat([
                                                                               known_embedding,
                                                                               tf.expand_dims(
            ...
            tuple_state = tuple([tf.contrib.rnn.LSTMStateTuple(
                unstack_state[idx][0], unstack_state[idx][1]) for idx in
                                 range(num_layers)])
            ...
            with tf.variable_scope('layer1'):
                local_w1 = tf.Variable(
                    tf.truncated_normal([lstm_size, 2 * lstm_size],
                                        stddev=0.1))
                local_b1 = tf.Variable(tf.zeros(2 * lstm_size))

            local_1_layer = tf.nn.relu(
                tf.matmul(distant_embedding, local_w1) + local_b1)

            with tf.variable_scope('layer2'):
                local_w2 = tf.Variable(
                    tf.truncated_normal([2 * lstm_size, lstm_size],
                                        stddev=0.1))
                local_b2 = tf.Variable(tf.zeros(lstm_size))

            local_2_layer = tf.nn.relu(
                tf.matmul(local_1_layer, local_w2) + local_b2)
            ...
            self.heuristics_input = tf.placeholder(tf.float32, [batch_size],
                                                   name="heuristics_input")

            self.time_input = tf.placeholder(tf.float32, [batch_size],
                                             name="time_input")

            #      half_batch_size = tf.div(self.heuristics.shape[0], 2)
            print(
                tf.slice(self.heuristics, [0, 0], [TRAIN_BATCH_SIZE, 1]).shape,
                tf.slice(self.heuristics, [TRAIN_BATCH_SIZE, 0],
                         [TRAIN_BATCH_SIZE, 1]).shape)
            margin = 1.0 - tf.slice(self.heuristics, [0, 0],
                                    [TRAIN_BATCH_SIZE, 1]) + tf.slice(
                self.heuristics, [TRAIN_BATCH_SIZE, 0], [TRAIN_BATCH_SIZE, 1])
